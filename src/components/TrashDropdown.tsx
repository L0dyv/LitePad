...
export function TrashDropdown({ closedTabs, onRestore, onDelete, onClear }: TrashDropdownProps) {
    ...
    const handleRestore = (e: React.MouseEvent, tab: ClosedTab) => {
        e.stopPropagation()
        onRestore(tab)
    ...
    const handleDeleteClick = (e: React.MouseEvent, tab: ClosedTab) => {
        e.stopPropagation()
    ...
    return (
        <>
                ...
                {isOpen && (
                    <div className="trash-menu">
                        <div className="trash-header">
                            ...
                            {closedTabs.length > 0 && (
                                <button className="trash-clear" onClick={handleClearClick}>
                                    {t('trash.clear')}
                                </button>
                            )}
                        </div>
                        <div className="trash-list">
                            {closedTabs.length === 0 ? (
                                ...
                                closedTabs.slice(0, 10).map((tab) => {
                                    const tabKey = tab.id + tab.closedAt
                                    return (
                                        <div key={tabKey} className="trash-item">
            ...
            <ConfirmDialog
                ...
                title={confirmDialog?.type === 'clear' ? t('trash.clearTitle') : t('trash.deleteTitle')}
                message={confirmDialog?.type === 'clear'
                    ? t('trash.clearMessage')
