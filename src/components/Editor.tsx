...
class TaskCheckboxWidget extends WidgetType {
    ...
    toDOM(view: EditorView) {
        ...
        checkbox.addEventListener('click', (e) => {
            ...
            const current = view.state.doc.sliceString(this.togglePos, this.togglePos + 1)
            const isChecked = current.toLowerCase() === 'x'
            view.dispatch({
                changes: { from: this.togglePos, to: this.togglePos + 1, insert: isChecked ? ' ' : 'x' }
            })
        })
...
const taskListPlugin = ViewPlugin.fromClass(class {
    ...
    buildDecorations(view: EditorView): DecorationSet {
        ...
        for (let i = 1; i <= doc.lines; i++) {
            ...
            builder.add(bracketFrom, bracketTo, Decoration.replace({
...
const tableBorderPlugin = ViewPlugin.fromClass(class {
    decorations: DecorationSet
    ...
    buildDecorations(view: EditorView): DecorationSet {
        ...
        for (let lineNo = 1; lineNo <= doc.lines; lineNo++) {
            ...
            for (let bodyLineNo = lineNo + 1; bodyLineNo <= doc.lines; bodyLineNo++) {
        ...
        for (const lineNo of sortedLineNos) {
            ...
            builder.add(line.from, line.from, Decoration.line({ class: kind === 'sep' ? 'cm-table-sep' : 'cm-table-row' }))

            const text = line.text
            for (let i = 0; i < text.length; i++) {
                if (text[i] !== '|') continue
                builder.add(line.from + i, line.from + i + 1, Decoration.mark({ class: 'cm-table-pipe' }))
            }
        }
...
export function Editor({ content, onChange, onActivity, font = 'Consolas', fontSize = 14, autoFocus = false, tabIndentText = '    ', enableCodeBlockHighlight = false, enableQuickSymbolInput = true, jumpTo = null, onJumpApplied }: EditorProps) {
    ...
    useEffect(() => {
        ...
        const getSelectedLineRange = (doc: EditorState['doc'], from: number, to: number) => {
        ...
        const getLeadingWhitespace = (lineText: string) => lineText.match(/^[\t ]*/)?.[0] ?? ''
        ...
        const getLineIndentDeltaColumns = (lineText: string, lineNo: number, direction: 'indent' | 'outdent', affectedLines: Set<number>) => {
        ...
        const getOrderedListRenumberChanges = (doc: EditorState['doc']) => {
            ...
            const unwindForNonOrderedLine = (indentCols: number) => {
                while (orderedStack.length > 0 && indentCols <= orderedStack[orderedStack.length - 1].indentColumns) {
                    orderedStack.pop()
                }
            }
            ...
            for (let lineNo = 1; lineNo <= doc.lines; lineNo++) {
                ...
                if (orderedStack.length === 0 || indentCols > orderedStack[orderedStack.length - 1].indentColumns) {
                ...
                changes.push({ from: numberFrom, to: numberTo, insert: String(desiredNumber) })
            }
        ...
        const handleQuickSymbolSpace = (view: EditorView) => {
            ...
            if ((marker.startsWith('#') || marker === '>') && indentCols > 3) return false
        ...
        const markdownExtension = markdown({
            ...
            extensions: [GFM],
            ...(enableCodeBlockHighlight ? { codeLanguages: codeFenceLanguages } : {})
        })
        ...
        const handleTabIndent = (direction: 'indent' | 'outdent') => {
            ...
            for (const range of state.selection.ranges) {
                if (range.from !== range.to) {
                    ...
                    for (let lineNo = lineRange.startLineNo; lineNo <= lineRange.endLineNo; lineNo++) {
                ...
                // 列表/引用：整行缩进
                if (lineType === 'quote' || lineType === 'olist' || lineType === 'ulist' || lineType === 'task') {
                    affectedLines.add(lineNo)
                    continue
                }
            ...
            const shouldRenumberOrderedLists = Array.from(affectedLines).some((lineNo) => !insideCodeBlock[lineNo] && getLineType(doc.line(lineNo).text) === 'olist')
            if (shouldRenumberOrderedLists) {
                ...
                for (let lineNo = 1; lineNo <= doc.lines; lineNo++) {
                    ...
                    const lineText = line.text
                    const ws = getLeadingWhitespace(lineText)
                    const indentCols = Math.max(0, getIndentColumns(ws) + getLineIndentDeltaColumns(lineText, lineNo, direction, affectedLines))
                ...
                EditorView.updateListener.of((update) => {
                    ...
                    if (!isAutoRenumbering.current) {
                        const shouldTriggerRenumber = update.transactions.some((tx) => {
                            const isRelevantInput = tx.isUserEvent('input') && !tx.isUserEvent('input.paste') && !tx.isUserEvent('input.drop')
                            const isRelevantDelete = tx.isUserEvent('delete')
                            return (isRelevantInput && hasInsertedNewline(tx)) || (isRelevantDelete && hasRemovedNewline(tx))
                        })

                        if (shouldTriggerRenumber) {
                            const cursorLineText = update.state.doc.lineAt(update.state.selection.main.head).text
                            if (/^[\t ]*\d+\.\s+/.test(cursorLineText)) {
                                const renumberChanges = getOrderedListRenumberChanges(update.state.doc)
